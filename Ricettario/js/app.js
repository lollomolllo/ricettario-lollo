...
// --- 2. LA FUNZIONE CHE ACCENDE LA NAVBAR ---
function inizializzaNavbar() {
    ...
    // Qui diciamo ai bottoni cosa fare quando li clicchi
    navElenco.addEventListener('click', (e) => { e.preventDefault(); setActiveNav(navElenco); UI.renderElenco(); initElenco(); });
    navCalendario.addEventListener('click', (e) => { e.preventDefault(); setActiveNav(navCalendario); UI.renderCalendario(); initCalendario(); });
    navSpesa.addEventListener('click', (e) => { e.preventDefault(); setActiveNav(navSpesa); UI.renderSpesa(); initSpesa(); });
    navInserimento.addEventListener('click', (e) => { e.preventDefault(); setActiveNav(navInserimento); UI.renderInserimento(); initInserimento(); });
    navImpostazioni.addEventListener('click', (e) => { e.preventDefault(); setActiveNav(navImpostazioni); UI.renderImpostazioni(); initImpostazioni(); });
